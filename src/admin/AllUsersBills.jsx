...
function Bills() {
  ...
  const deleteBill = async (billNumber) => {
    ...
    try {
      ...
      if (data.success) {
        setBills((prev) =>
          prev.filter((bill) => bill.billNumber !== billNumber)
        );

        // Close details if the deleted bill was expanded
  ...
  const totalPages = Math.ceil(
    filteredBills.length / resultsPerPage
  );
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        <button
                          ...
                          className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-xs sm:text-sm transition transform hover:scale-105"
                        >
                          Delete
                        </button>
                      </div>
