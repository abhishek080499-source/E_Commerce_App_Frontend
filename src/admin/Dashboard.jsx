...
async function saveEdit(id) {
  ...
  filteredProducts.forEach((p) => {
    ...
    categoryMap.get(catName).totalPrice +=
      Number(p.price) || 0;

    categoryMap.get(catName).totalQuantity +=
  ...
  const categoryQuantityValues = Array.from(
    categoryMap.values()
  ...
  const animatedTotalProducts = useCountUp(
    totalProducts,
    1200
  );

  const animatedHighestPrice = useCountUp(
    highestPrice,
    1200
  );

  const animatedAveragePrice = useCountUp(
    Number(averagePrice),
    1200
  );

  const animatedTotalRevenue = useCountUp(
    totalRevenue,
    1200
  );
